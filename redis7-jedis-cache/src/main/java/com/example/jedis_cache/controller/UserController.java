package com.example.jedis_cache.controller;
...
public class UserController {
    ...
    public String getUserEmail(@PathVariable Long id) {
        try (Jedis jedis = jedisPool.getResource()){
            ...
            // 2. else db
            userEmail = userRepository.findById(id).orElse(UserEntity.builder().build()).getEmail();
            // 3. cache
...
//        return userRepository.findById(id).orElse(UserEntity.builder().build()).getEmail();
    }
}
